...
class DiceAgent
  ...
  def process_natural_language(text)
    ...
    return handle_dice_request(text) if text.match?(/roll|dice|die/)

    # Parse prime checking requests
    return handle_prime_request(text) if text.match?(/prime|check/)

    # Parse statistics requests
    return handle_stats_request(text) if text.match?(/stat|history|average/)
  ...
  def prime_explanation(num)
    ...
    if prime?(num)
      ...
      factor = (3..Math.sqrt(num)).step(2).find { |i| (num % i).zero? }
      if factor
  ...
  def handle_dice_request(text)
    ...
    rolls_text = if result[:rolls].length == 1
  ...
  def handle_prime_request(text)
    ...
    message_parts << "✨ Prime: #{prime_numbers.join(', ')}" if prime_numbers.any?

    message_parts << "❌ Not prime: #{non_prime_numbers.join(', ')}" if non_prime_numbers.any?
